...
export default function MobileRecordPage() {
  ...
  const cleanupStream = useCallback(() => {
    if (streamRef.current) {
        ...
        console.log("Stream parado e câmera liberada.");
    }
  ...
  const getPermissionsAndDevices = useCallback(async () => {
    ...
    try {
      ...
      // Parar imediatamente a stream temporária, só queríamos a permissão
      tempStream.getTracks().forEach(track => track.stop());

      // Agora que temos permissão, podemos enumerar os dispositivos
  ...
  }, [recordingState, recordedBlob, getStream, permission]);
  ...
  const handleSave = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    console.log("[handleSave] Iniciando salvamento da gravação.");
    ...
    if (!title || !recordedBlob || !session?.uid || !database) {
      ...
      setRecordingState('stopped');
      console.error("[handleSave] Falha: Título, blob ou sessão do usuário ausentes.");
      return;
    }
    ...
    try {
        ...
        console.log(`[handleSave] ID da memória gerado: ${memoryId}.`);
        ...
        console.log("[handleSave] Upload concluído. Salvando metadados no banco de dados.");
        ...
        if (result && result.success) {
            console.log("[handleSave] Sucesso! Memória salva.");
        ...
        console.error("[handleSave] Erro capturado no fluxo de salvamento:", error);
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className={cn("relative w-full aspect-video bg-muted rounded-lg overflow-hidden flex items-center justify-center")}>
            ...
            {mode === 'audio' && permission === 'granted' && !recordedBlob && recordingState === 'idle' && (
                 <div className="absolute inset-0 flex flex-col items-center justify-center text-muted-foreground bg-muted">
          ...
          {mode === 'video' && videoDevices.length > 1 && frontCamera && backCamera && (
            <div className="flex justify-center gap-4">
                <Button
                    ...
                    onClick={() => {
                        setCurrentDeviceId(frontCamera.deviceId);
